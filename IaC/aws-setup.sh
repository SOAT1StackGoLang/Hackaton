...
# create the bucket
aws s3api create-bucket --bucket s1sg-tc-terraform-state-bucket-hackaton --region us-east-1
...
echo "terraform init -backend-config=\"bucket=s1sg-tc-terraform-state-bucket-hackaton\" -backend-config=\"key=terraform.tfstate\" -backend-config=\"region=us-east-1\" -reconfigure"
